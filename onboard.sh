      ...
      case "$dest_rel" in
        ...
        .github/workflows/*.yml)
          ...
          if [ -n "$wf_canon" ]; then
            ...
            # Inert-blind count (see rc_strip_inert): comments and blank
            # lines are for humans; the pipeline runs the rest. Without this
            # the template's origin repo read ~90 stale lines over 2 real
...
if [ -n "$PREFLIGHT" ]; then
  ...
  # CLAUDE.md is read at the start of every run as the repo's architectural
  # reference. A copy still carrying the template's onboarding-checklist section
  # is a stub, not a reference — the "Key files" and conventions sections are
  # unfilled, so every run greps blindly. Detect the checklist HEADER, not
  # `- [ ]` lines: the entry-format example further down legitimately contains
  # one, and a filled CLAUDE.md that kept that example must not warn forever.
  # Leftover `{{…}}` is a separate, sharper failure: subst missed a placeholder.
  if [ -f "$TARGET/CLAUDE.md" ]; then
    if grep -q '^## Onboarding checklist' "$TARGET/CLAUDE.md" 2>/dev/null; then
      PF_WARN+=("CLAUDE.md still carries the onboarding checklist — Key files and conventions are unfilled, so every run reads a stub; fill them and delete that section")
    fi
    if grep -qE '\{\{[A-Z_]+\}\}' "$TARGET/CLAUDE.md" 2>/dev/null; then
      PF_WARN+=("CLAUDE.md has unfilled {{…}} placeholders — onboard.sh's subst missed them; fill by hand")
    fi
  fi
